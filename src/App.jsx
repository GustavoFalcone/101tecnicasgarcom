...
function Trust() {
  return <div className="trust">{[["lock", "PAGAMENTO SEGURO"], ["celular", "ACESSO DIGITAL"], ["check", "SEM MENSALIDADE"]].map(([i, s]) => <span key={s}><Icon name={i} />{s}</span>)}</div>;
}
...
<section className="hero reading" data-section="hero"><span className="eyebrow heroLabel">MATERIAL ILUSTRADO</span><h1><span>+101 Técnicas para servir como um Garçom Profissional</span><span>Para entender como agir no salão com mais</span><span>clareza, agilidade e segurança.</span></h1><p className="heroSubtitle">Um material visual e direto para quem está começando ou já trabalha como garçom e quer <strong>consultar, entender e aplicar técnicas de serviço no salão de forma muito mais prática.</strong></p><Asset src="/assets/hero-produto.webp" alt="+101 Técnicas para Servir como um Garçom Profissional" className="heroAsset" hero /><CTA>QUERO ACESSAR AS +101 TÉCNICAS</CTA><p className="micro">Material 100% digital • Use no celular ou imprima • Pagamento único</p></section>
...
<section className="section reading summary reveal" data-section="summary"><Asset className="peopleAsset" src="/assets/plano-completo-pessoas.webp" alt="Plano Completo das +101 Técnicas do Garçom" /><h2>Veja tudo o que você terá acesso imediato ao escolher o material completo:</h2><div className="summaryCards"><article><Icon name="acesso-tecnicas-garcom" /><div><h3>101+ TÉCNICAS DO GARÇOM</h3><p>Técnicas práticas sobre bandejas, pratos, bebidas, postura, atendimento e recolhimento.</p></div></article>{bonuses.map((b) => <article key={b.title}><Icon name={b.icon} /><div><h3>{b.summaryTitle}</h3><p>{b.summary}</p></div></article>)}</div><p className="micro">Tudo em formato digital • Consulte no celular • Imprima as páginas que quiser</p></section>
<div className="reading intermediate" data-section="intermediate"><CTA>VER PLANOS</CTA></div>
<section className="section reading benefits reveal" data-section="benefits"><h2>O QUE MUDA NA PRÁTICA?</h2><p className="intro">Em vez de depender só da experiência ou do improviso, você consulta uma técnica visual e entende rapidamente como executar melhor aquela situação no salão.</p><div className="benefitCards">{benefits.map((b, i) => <article key={b.title}><div className="benefitVisual"><span>0{i + 1}</span><Icon name={b.icon} /></div><div><h3>{b.title}</h3><p>{b.text}</p></div></article>)}</div></section>
<section className="section reading bonuses reveal" data-section="bonuses"><span className="eyebrow">4 BÔNUS NO PLANO COMPLETO</span><h2>Além das técnicas, veja o que você também vai receber.</h2><p className="intro">Além das +101 Técnicas do Garçom, você recebe quatro materiais adicionais para aprofundar a parte prática e visual.</p><div className="bonusCards">{bonuses.map((b, i) => <article key={b.title}><span className="bonusNumber">BÔNUS 0{i + 1}</span><Asset className="bonusAsset" src={b.image} alt={b.title} /><div className="bonusCopy"><h3>{b.title}</h3><p>{b.text}</p><div className="bonusPrice"><del>DE {b.value}</del><strong>GRÁTIS</strong></div></div></article>)}</div></section>
<section className="reading calculatorImage reveal" data-section="bonus-value"><img src="/assets/generated/calculadora-pizza.webp" width="985" height="1596" loading="lazy" decoding="async" alt="Somando tudo o que você vai levar. Atlas Visual de Utensílios e Serviço: R$ 27,00. +30 Desafios do Garçom: R$ 23,00. Dicionário Visual do Restaurante: R$ 17,00. Certificado de Conclusão: R$ 20,00. Valor total dos bônus: R$ 87,00. Hoje, os bônus saem por R$ 0,00." /></section>
...
<article className="plan basic" data-section="basic"><span className="eyebrow">PAGAMENTO ÚNICO</span><h3>Plano Básico</h3><p className="planDescription">Para quem quer começar apenas com o material principal.</p><div className="price">{offer.basicPrice}</div><List items={["+101 Técnicas para Servir como um Garçom Profissional", "Acesso digital imediato", "Imprimível"]} /><div className="planAction"><CTA onClick={() => setUpgrade(true)}>QUERO SOMENTE O PLANO BÁSICO</CTA><Trust /><a className="upgradeHint" href="#plano-completo"><Icon name="arrow" /><span>Temos uma oferta especial para você</span><Icon name="arrow" /></a></div></article>
<Upgrade open={upgrade} close={() => setUpgrade(false)} />
<article className="plan complete" id="plano-completo" data-section="complete"><span className="popular">MAIS ESCOLHIDO</span><h3>Plano Completo</h3><p className="planDescription">Leve o material principal com todos os bônus complementares</p><Asset className="planAsset" src="/assets/planos/plano-completo.webp" alt="Plano Completo com todos os bônus" /><del className="anchorPrice">{offer.completeAnchor}</del><span className="por">POR:</span><div className="price">{offer.completePrice}</div><List items={["+101 Técnicas para Servir como um Garçom Profissional", "Acesso digital imediato em PDF", "Acesso vitalício", "Pagamento único"]} /><div className="includedBonuses imageBonus"><img src="/assets/generated/exclusive-bonuses.webp" alt="+4 bônus exclusivos: Atlas Visual de Utensílios, +30 Desafios do Garçom, Dicionário Visual do Restaurante e Certificado de Conclusão." loading="lazy" decoding="async" /></div><div className="planAction"><Checkout>QUERO O PLANO COMPLETO</Checkout><Trust /></div></article></div></section>
<section className="section reading guarantee reveal" data-section="guarantee"><div className="guaranteeSeal imageSeal"><img src="/assets/generated/garantia-premium.webp" alt="Símbolo de compra protegida" loading="lazy" decoding="async" /></div><h2>Conheça o material por 7 dias.</h2><p>Depois da compra, você pode acessar as técnicas e avaliar o conteúdo com calma. Se decidir que o material não atende ao que esperava, solicite o reembolso dentro desse prazo pelas regras da plataforma de pagamento.</p></section>
<FAQ /><section className="section finalCta" data-section="final"><div className="reading"><Icon name="rodape-emblema-garcom" /><h2>Tenha uma referência sempre por perto</h2><p>Consulte as +101 técnicas quando quiser revisar um movimento, conferir uma postura ou se preparar melhor para o serviço.</p><CTA>QUERO ACESSAR AS +101 TÉCNICAS</CTA></div></section>
</main><footer data-section="footer"><div className="reading"><p>+101 Técnicas para Servir como um Garçom Profissional é um material digital de consulta e apoio à rotina no salão. As orientações podem ser adaptadas aos padrões de cada estabelecimento e não substituem treinamentos obrigatórios, normas internas ou exigências sanitárias.</p><p>© 2026 +101 Técnicas para Servir como um Garçom Profissional</p></div></footer></>;
}
